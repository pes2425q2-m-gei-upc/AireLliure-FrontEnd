...
class RutaViewModel : ViewModel() {
    ...
    fun get_informacio_ruta(id_ruta: Int)= viewModelScope.launch {
        try{
            ...
            println("inforuta:" +all_info_ruta)
            println("chatpgt"+resposta.toString())
        } catch(e:Exception){
            println("Error, no s'han carregat les dades correctament")
        }
    }
    fun afegir_valoracio(user_id: Int, ruta_id: Int, puntuacio_:Float, comentari_: String)=viewModelScope.launch {
        try{
